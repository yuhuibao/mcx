...
#pragma omp master
{
	   ...
	   CUDA_ASSERT(hipMemcpy(gPseed, Pseed, sizeof(RandType)*RAND_BUF_LEN,  hipMemcpyHostToDevice));
           ...
           hipLaunchKernelGGL(mcx_test_rng, dim3(1), dim3(1), 0, 0, gfield,gPseed);
           tic1=GetTimeMillis();
...
#pragma omp master
{
	   ...
	   switch(isinternal*100 + (cfg->isreflect>0)*10+(cfg->issavedet>0)){
	       case 0:  hipLaunchKernelGGL(HIP_KERNEL_NAME(mcx_main_loop<0,0,0>), dim3(mcgrid), dim3(mcblock), sharedbuf, 0, gmedia,gfield,genergy,gPseed,gPpos,gPdir,gPlen,gPdet,gdetected,gsrcpattern,greplayw,greplaytof,greplaydetid,gseeddata,gdebugdata,gprogress);break;
...
//#endif
	     ...
	     do{
//#ifndef WIN32
//               CUDA_ASSERT(hipEventQuery(updateprogress));
//#endif
               ndone = *progress;
	       if (ndone > p0){
